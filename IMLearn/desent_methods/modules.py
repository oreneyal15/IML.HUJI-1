...
class LogisticModule(BaseModule):
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray, **kwargs) -> np.ndarray:
        ...
        """
        # raise NotImplementedError()
        sigmoid = 1 / (1 + np.exp(-self.weights @ X.T))
        return -(y @ X - X.T @ sigmoid) / X.shape[0]
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        return self.fidelity_module_.compute_output(**kwargs, waights=self.weights) + \
            self.lam_ * self.regularization_module_.compute_output(**kwargs, waights=self.weights)
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        # raise NotImplementedError()
        reg_jacobian = self.regularization_module_.compute_jacobian(**kwargs, waights=self.weights)
        if self.include_intercept_:
            reg_jacobian = np.insert(reg_jacobian, 0, 0)
        return self.fidelity_module_.compute_jacobian(**kwargs, waights=self.weights) + \
               self.lam_ * reg_jacobian
